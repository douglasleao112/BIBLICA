window.BENCAO_I18N = (() => {
  ...
  const copy = {
    pt: {
      pageTitle:'Um presente de bênção', description:'Crie uma mensagem de bênção personalizada para alguém de quem gosta.', ribbon:'Elabore um card surpresa para quem amas', progress:'Progresso do presente', panel:'Criação do presente', modal:'Exemplo de presente. Toque na imagem para fechar', example:'Ver exemplo {number}', exampleAlt:'Exemplo de presente {number}',
      imageRights:'Ao enviar uma fotografia, confirme que tem autorização para a utilizar e partilhar, sobretudo se retratar uma criança.', termsLabel:'Termos de uso', termsCopy:'Use apenas imagens cuja utilização está autorizada. A fotografia é enviada ao serviço de geração para criar o cartão. A arte gerada por IA pode conter diferenças; reveja-a antes de a partilhar.', copyright:'© {year} Viva La Vida. Todos os direitos reservados.',
    ...
    es: {
      pageTitle:'Un regalo de bendición', description:'Crea un mensaje de bendición personalizado para alguien que amas.', ribbon:'Crea una tarjeta sorpresa para quien amas', progress:'Progreso del regalo', panel:'Creación del regalo', modal:'Ejemplo de regalo. Toca la imagen para cerrar', example:'Ver ejemplo {number}', exampleAlt:'Ejemplo de regalo {number}',
      imageRights:'Al enviar una fotografía, confirma que tienes autorización para usarla y compartirla, especialmente si muestra a un menor.', termsLabel:'Términos de uso', termsCopy:'Usa solo imágenes cuyo uso esté autorizado. La fotografía se envía al servicio de generación para crear la tarjeta. La imagen generada por IA puede contener diferencias; revísala antes de compartirla.', copyright:'© {year} Viva La Vida. Todos los derechos reservados.',
  ...
  function applyStatic() {
    ...
    document.querySelector('#image-rights').textContent = t('imageRights');
    document.querySelector('#terms-label').textContent = t('termsLabel');
    document.querySelector('#terms-copy').textContent = t('termsCopy');
    document.querySelector('#copyright').textContent = format('copyright', { year: new Date().getFullYear() });
  }
