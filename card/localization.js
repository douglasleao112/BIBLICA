window.BENCAO_I18N = (() => {
  ...
  const copy = {
    pt: {
      ...
      introKicker:'Uma surpresa especial', introHeading:'Uma <em class="hero-accent">bênção</em> para alguém que <em class="hero-accent">ama</em>.', introText:'Escolha personagens bíblicas e crie uma mensagem especial para alguém de quem gosta.', introNote:'Atenção: um cartão personalizado por pessoa.',
    ...
    es: {
      ...
      introKicker:'Una sorpresa especial', introHeading:'Una <em class="hero-accent">bendición</em> para alguien que <em class="hero-accent">amas</em>.', introText:'Elige personajes bíblicos y crea un mensaje especial para alguien que amas.', introNote:'Atención: una tarjeta personalizada por persona.',
  ...
  function start() {
    ...
    window.BENCAO_TRACKER.getGeo().then(geo => {
      ...
      if (code) setLanguage(forced || (LATAM.has(code) ? 'es' : 'pt'), code);
    });
  }

  function chooseCountry(code) {
    if (code && !forced) setLanguage(LATAM.has(code) ? 'es' : 'pt', code);
  }

  return { t, format, start, chooseCountry, onChange:listener => listeners.add(listener), get lang() { return lang; }, get countryCode() { return countryCode; }, isLatinAmerica:code => LATAM.has(code) };
})();
